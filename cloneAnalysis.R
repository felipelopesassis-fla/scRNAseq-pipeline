...
n_distinct(clone_celltype_counts$clone_id)
# clones with > 1 cell
sum(clone_celltype_counts$n > 1)
# clones with >= 10 cells
sum(clone_celltype_counts$n >= 10)
# clones with > 1 cell type
mean(clone_celltype_counts$ntypes > 1)
# clones with > 1 cell type among clones with > 1 cell
mean(filter(clone_celltype_counts, n > 1)$ntypes > 1)
...
comb_germline %>%
  ...
  mutate(s2p = clone_id %in% s2pclones) %>%
  group_by(s2p) %>%
  summarize(ncells = n_distinct(new_barcode)) %>%
  ungroup() %>%
  summarize(number = ncells[s2p], total=ncells[s2p] + ncells[!s2p],
    mean=number/(ncells[s2p] + ncells[!s2p]))
...
comb_germline %>%
  ...
  summarize(n_donor = n_distinct(Donor),
    ...
    ighv=paste(unique(getGene(v_call)),collapse=",")) %>%
  filter(n_donor == 3) %>%
...
comb_germline$population = paste(comb_germline$Donor, 
  comb_germline$group, comb_germline$timepoint, sep="|")
...
sample_curve$group = unlist(lapply(strsplit(sample_curve$population,
...
print(ggplot(sample_curve, aes(x=timepoint, y=d, group=group, color=group)) + 
    geom_line() + geom_point() + 
    geom_errorbar(aes(ymin=d_lower,ymax=d_upper),width=0.2) +
    facet_wrap(. ~ Donor, scales="free_y")+theme_bw() +
    ylab("Simpson's diversity") + xlab("Timepoint")+
...
clustercounts = comb_germline %>%
    filter(!naive) %>%
    group_by(Donor, sort, clone_id, timepoint) %>%
    summarize(n = n_distinct(new_barcode))
timepoints = levels(comb_germline$timepoint)
results = tibble()
for(d in unique(comb_germline$Donor)){
  print(d)
  for(s in unique(comb_germline$sort)){
    for(a in timepoints){
        for(b in timepoints){
          clone_counts = clustercounts %>%
            filter(Donor == d & sort == s) %>%
  ...
  for(a in timepoints){
    for(b in timepoints){
      if(which(timepoints==a) > which(timepoints==b)){
        values = filter(temp, !!a==a & !!b==b & sort=="S2pos")
      }else if(which(timepoints == a) < which(timepoints == b)){
        values = filter(temp, !!a==a & !!b==b & sort=="S2neg")
      }else{
        sp = filter(temp, !!a==a & !!b==b & sort=="S2pos")
        sn = filter(temp, !!a==a & !!b==b & sort=="S2neg")
...
clustercounts = comb_germline %>%
    group_by(Donor, sort, clone_id, new_celltype) %>%
    summarize(n = n_distinct(new_barcode))
...
# Plot getting C5.2 - RM clones at 6 months
...
m6 = filter(comb_germline, clone_id %in% multi_time)
m6$overlap = "No C5.2, M6"
m6$overlap[m6$clone_id %in% mem6_clones] = "C5.2, M6"

# calculate frequency of each celltype at each timepoint
cluster_time_frequency = m6 %>%
  filter(!is.na(new_celltype)) %>%
  group_by(Donor, s2clone, overlap, timepoint, new_celltype) %>%
  summarize(n=n_distinct(new_barcode)) %>%
  mutate(freq = n/sum(n))

cluster_time_frequency$timepoint = factor(cluster_time_frequency$timepoint, levels=ovn)
...
cluster_time_frequency$S2 = "S-2P- clones"
cluster_time_frequency$S2[cluster_time_frequency$s2clone] = "S-2P+ clones"
cluster_time_frequency$S2 = factor(cluster_time_frequency$S2, 
  levels=c("S-2P- clones", "S-2P+ clones"))
...
m6$period = "Early"
m6$period[m6$timepoint == "M6"] = "M6"
m6$S2 = "S-2P-"
m6$S2[m6$s2clone] = "S-2P+"
mufreq_time = m6 %>%
...
# Check SHM of C5.2 - RM B cells at 6mo
m6$period_celltype = as.character(m6$new_celltype)
m6$period_celltype[m6$period=="Early"] = "All"
mufreq_time_agg = m6 %>%
...
agg = tibble()
for(celltype in unique(m6$new_celltype)){
...
convergent_counts$new_celltype = factor(convergent_counts$new_celltype, levels=level.order2)
...
#  genes$convergent_cluster_donors = paste(genes$convergent_cluster_donors, "donor")
#  genes$convergent_cluster_donors[genes$convergent_cluster_donors =="2 donor"] = "2 donors"
#  genes$convergent_cluster_donors[genes$convergent_cluster_donors =="3 donor"] = "3 donors"
  ...
  #colors = c("1 donor"="#33a02c", "2 donors"="#ff7f00", "3 donors"="#6a3d9a",
  #  "CoV-AbDab"="black")
